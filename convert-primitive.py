...
if not show_gui:
    ...
    parser.add_argument('-c', '--compress', dest='compress', action='store_true',
                        help='Compress output using zlib')
    parser.add_argument('-nm', '--nomtl', dest='no_mtl', help='don\'t output material', action='store_true')
    parser.add_argument('-nvt', '--novt', dest='no_vt', help='don\'t output UV coordinates',
                        action='store_true')
    ...
    parser.add_argument('-f', '--format', dest='format', help='output format, obj or collada',
...
# GUI
    ...
    def unpack_file():
        if py3k:
            filename_primitive = filedialog.askopenfilename(
                filetypes=(('Primitives', '*.primitives*'), ('all files', '*.*')))
        else:
            filename_primitive = tkFileDialog.Open(root, filetypes=(
            ('Primitives', '.primitives*'), ('all files', '*.*'))).show()
    ...
    check_output_material = ttk.Checkbutton(root, text='output material', variable=root.output_material,
